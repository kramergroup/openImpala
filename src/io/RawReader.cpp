...
RawReader::RawReader( std::string const& filename) : m_filename(filename)
{
  readRawFile();
}

void RawReader::readRawFile()
{
...
uint32_t RawReader::depth()
{
  return m_depth;
}

uint32_t RawReader::height()
{
  return m_height;
}

uint32_t RawReader::width()
{
  return m_width;
}

amrex::Box RawReader::box()
{
  amrex::Box box(amrex::IntVect{0,0,0}, amrex::IntVect{m_width-1,m_height-1,m_depth-1});
  return box;
}

void RawReader::threshold(const uint32_t threshold, amrex::iMultiFab& mf)
{
